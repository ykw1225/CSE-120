/*
 * write101.c, edited version of write 10, take file name and process id as input argument, write to different file according to processID 
 *
...
//printf("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAARG1: %s\n", argv[1]);
...
//printf("File Name!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!:%s\n", file);
    ...
    do_write (file1, str, len, len);
    do_validate (file1, buffer, str, len);

    /* write 8 bytes at a time */
    do_write (file1, str, len, 8);
    do_validate (file1, buffer, str, len);

    /* write 1 byte at a time */
    do_write (file1, str, len, 1);
    do_validate (file1, buffer, str, len);
    ...
    /* write all at once */
    do_write (file9, (char *) bigbuf1, len, len);
    do_validate (file9, (char *) bigbuf2, (char *) bigbuf1, len);

    /* write 128 bytes at a time */
    do_write (file9, (char *) bigbuf1, len, 128);
    do_validate (file9, (char *) bigbuf2, (char *) bigbuf1, len);
